...
const call = (body = payload, token = 'valid') => POST(new NextRequest('https://clipwise.ca/api/send-email', { method: 'POST', headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}), 'x-internal-secret': 'dummy-cron' }, body: JSON.stringify(body) }));
(async () => {
  ...
  for (const type of ['signup_code', 'subscription_card_updated', 'owner_weekly_digest', 'connect_reminder', 'password_reset', 'barber_password_reset', 'barber_invite']) {
