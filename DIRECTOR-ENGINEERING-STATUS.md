...
Essential service/barber read failure reproduced as an empty menu; bounded loader/cleanup/readiness fix and optional-review isolation implemented. Focused actual-effect/handler tests pass. Targeted lint remains 18 pre-existing unused errors and 4 img warnings; ESLint HEAD lintText versus modified lintFiles normalized rule/severity/message arrays match exactly. No new diagnostics; do not claim lint passed. Full flow suite and real isolated production build passed with dummy credentials; pushed `b679b1f` and remote main verified. Director accepted documented pre-existing lint baseline for this bounded change. Files: `src/app/book/[shopslug]/booking-client.tsx`, `src/app/book/[shopslug]/page.tsx`, `scripts/tests/public-booking-load-check.cjs`, runner, `PUBLIC-BOOKING-LOAD-2026-09-18.md`. Next assigned batch: quick-add required reads, separate from uncertain-submit semantics.

## Quick-add prerequisites

Actual-effect failed/null reads reproduced as empty lists; loader/readiness/retry/scope guards now pass focused tests. Focused tests, settled full flow rerun and isolated/shared production builds passed with dummy credentials; pushed 54715f5 and remote verified. Earlier suite failed during concurrent email fixture edits; settled rerun passes. Files: `src/components/dashboard/add-appointment-modal.tsx`, `scripts/tests/quick-add-load-check.cjs`, runner, `QUICK-ADD-LOAD-2026-09-18.md`. Lint baseline comparison identical: 1 existing unused error and 2 existing shop dependency warnings, no new diagnostics; not a clean lint result. Submit ambiguity and duplicates remain a separate candidate, no automatic retry added.

Separate actual-submit reproduction (mock requests only): two held calls issue 2 requests and success callbacks; malformed 200 body {} closes/resets with Booked; thrown fetch rejects and leaves saving true. Server success shape is {id,status,barber_id}. Proposed next bounded guard/confirmation/uncertain-lock recovery sent to director, not implemented in this loader batch. No automatic retry or database idempotency change.


## Quick-add submit recovery

Director-authorized follow-up implemented and verified: synchronous pending/override guard, confirmed canonical ID, retained failed drafts, conservative uncertain lock, recognized no-write rejection retry and stale callback isolation. Focused handlers/lifecycle checks and full flows pass; unchanged lint baseline (1 error, 2 warnings), isolated dummy-credential production build passes (202 pages). Pushed c949a1a; exact remote main verified. Files: src/components/dashboard/add-appointment-modal.tsx, scripts/tests/quick-add-submit-check.cjs, quick-add-load-check.cjs fixture update, runner and QUICK-ADD-SUBMIT-2026-09-18.md. No server idempotency or live/deployment verification claimed.

## Client-profile save confirmation

Actual-handler zero-row false success reproduced in the remaining profile fields. Shared pending guard, confirmed client/shop update, persistent uncertain outcome guidance, conservative no-replay protection and stale selection/shop/unmount guards implemented for notes/hair/birthday/contact; loyalty excluded. Focused and full flow suite pass; targeted lint clean; isolated dummy-credential production build passes (202 pages). Pushed 2aec556; exact remote main verified. Files: src/app/dashboard/clients/page.tsx, scripts/tests/client-profile-confirmation-check.cjs, existing client-profile-save-check.cjs fixture, runner and CLIENT-PROFILE-CONFIRMATION-2026-09-18.md. No live/browser/deployment verification or atomicity/idempotency claim.
