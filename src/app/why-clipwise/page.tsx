    ...
    <div className="min-h-screen bg-background text-white">
        ...
        <div className="space-y-4">
          {squireProblems.map((item, i) => (
            <div key={i} className="bg-surface border border-border rounded-2xl p-5">
              ...
              <p className="text-xs text-[#777] italic ml-7 mb-3 border-l border-border pl-3">{item.quote}</p>
      ...
      <section className="py-20 px-4 text-center">
        <div className="max-w-xl mx-auto bg-surface border border-gold/20 rounded-3xl p-10 gold-glow">
          ...
          <p className="text-[#777] mb-6 text-sm">Join barbers who are done paying commissions, booking fees, and upsell traps. ClipWise is built for barbers — not for a marketplace.</p>
          <Link href="/signup">
            <Button size="lg" className="text-base px-8">Get Started Free <ArrowRight size={18} /></Button>
          </Link>
          <p className="text-xs text-[#777] mt-4">No credit card · No sales call · Cancel anytime</p>
        </div>
      </section>

      <footer className="border-t border-border py-8 px-4 text-center">
        <Logo size="sm" className="justify-center mb-2" />
        <p className="text-xs text-[#777]">© 2026 ClipWise Technologies Inc. · <Link href="/" className="hover:text-gold transition-colors">Back to home</Link></p>
      </footer>
    </div>
  );
}
