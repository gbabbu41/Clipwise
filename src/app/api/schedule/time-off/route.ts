...
async function authorize(token: string | undefined, barberId: string) {
  return authorizeSchedule(token, barberId, "request_time_off");
}
...
export async function POST(request: NextRequest) {
  ...
  if (!body || !validId(body.barber_id) || !validTimeOff(body)) {
  ...
  if (existingError) return NextResponse.json({ error: "Unable to load time off" }, { status: 503 });
  ...
  const overlaps = (existing ?? []).filter(r => {
    ...
    if (type === "blocked_hours") return r.start_time?.slice(0, 5) === newStart?.slice(0, 5) && r.end_time?.slice(0, 5) === newEnd?.slice(0, 5);
    return true;
  });
  ...
  if (covers) {
    ...
    if (upErr) return NextResponse.json({ error: "Unable to save time off" }, { status: 500 });
    // Preserve all existing coverage until the union has saved successfully.
    if (rest.length) {
      const { error: cleanupError } = await supabaseAdmin.from("time_off_requests").delete().in("id", rest.map(r => r.id as string)).eq("barber_id", barber.id);
      if (cleanupError) return NextResponse.json({ error: "Time off saved, but overlapping entries could not be consolidated. Refresh and retry." }, { status: 500 });
    }
    action = "merged";
    ...
    const { data: inserted, error: insertErr } = await supabaseAdmin
      ...
      .select("id")
      .single();
    if (insertErr) return NextResponse.json({ error: "Unable to save time off" }, { status: 500 });
  ...
  const { data: timeOff, error: listError } = await supabaseAdmin
  ...
  if (listError) return NextResponse.json({ error: "Time off saved, but the list could not refresh. Please reload." }, { status: 503 });
...
export async function DELETE(request: NextRequest) {
  ...
  if (!validId(id) || !validId(barberId)) return NextResponse.json({ error: "Invalid id" }, { status: 400 });
  const auth = await authorize(token, barberId);
  if ("error" in auth) return NextResponse.json({ error: auth.error }, { status: auth.status });

  const { data: entry, error: readError } = await supabaseAdmin.from("time_off_requests")
    .select("type").eq("id", id).eq("barber_id", barberId).maybeSingle();
  if (readError) return NextResponse.json({ error: "Unable to load time off" }, { status: 503 });
  if (!entry) return NextResponse.json({ error: "Time off not found" }, { status: 404 });
  if (!auth.isOwner && entry.type === "blocked_hours" && auth.barber.permissions?.block_hours === false) return NextResponse.json({ error: "No permission to block hours" }, { status: 403 });

  const { error } = await supabaseAdmin
    .from("time_off_requests").delete().eq("id", id).eq("barber_id", barberId);
  if (error) return NextResponse.json({ error: "Unable to remove time off" }, { status: 500 });
