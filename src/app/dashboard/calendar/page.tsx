...
const BARBER_DOT_PALETTE = [
  ...
  "bg-amber-400", "bg-cyan-400", "bg-fuchsia-400", "bg-lime-400",
];
...
function AgendaSheet({
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/60 z-40" onClick={onClose} />
      <div className="fixed right-0 top-0 bottom-0 w-full sm:w-[420px] bg-surface border-l border-border z-50 flex flex-col animate-fade-in">
        <div className="p-5 border-b border-border flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-400">{date.toLocaleDateString("en-CA", { year: "numeric" })}</p>
          ...
          {appts.map(appt => {
            ...
            return (
              <button key={appt.id} onClick={() => onOpenAppt(appt)}
                className="w-full text-left p-3 rounded-xl bg-surface-raised hover:bg-surface-raised/80 transition-colors flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between gap-2">
                    ...
                    <span className="text-xs text-gray-400 flex-shrink-0">{appt.time_slot}</span>
                  </div>
        ...
        <div className="p-4 border-t border-border">
          <button onClick={onDrillToDay}
            className="w-full py-2.5 rounded-xl bg-gold/15 hover:bg-gold/25 text-gold text-sm font-medium transition-colors">
            Open day view
          </button>
        </div>
      </div>
...
export default function CalendarPage() {
  ...
  const renderMonthView = () => {
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="grid grid-cols-7 border-b border-border bg-surface-raised/30">
          ...
          {visibleDays.map((day) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "border-r border-b border-border/40 p-1 sm:p-1.5 text-left flex flex-col gap-1 min-h-[72px] sm:min-h-[88px] transition-colors",
              ...
              >
                ...
                {isMobile ? (
                  ...
                  <div className="flex flex-wrap gap-0.5 overflow-hidden">
                    ...
                    {dayAppts.length > 6 && (
                      <span className="text-[9px] text-gray-400 leading-none">+{dayAppts.length - 6}</span>
                    )}
                  </div>
                ) : (
      ...
      return (
        <div ref={scrollRef} className="overflow-auto h-full">
          <div className="relative">
            {HOURS_24.map(hour => (
              <div key={hour} className="grid border-b border-border/20" style={{ gridTemplateColumns: `48px 1fr`, height: `${ROW_PX}px` }}>
    ...
    return (
      <div ref={scrollRef} className="overflow-auto h-full">
        <div className="min-w-[600px]">
          <div className="grid sticky top-0 z-10 bg-surface border-b border-border" style={{ gridTemplateColumns: `56px repeat(${cols.length}, 1fr)` }}>
            <div />
            {cols.map((b, i) => (
              <div key={b.id} className="px-3 py-3 text-center border-l border-border/40">
          ...
          <div className="relative">
            {HOURS_24.map(hour => (
              <div key={hour} className="grid border-b border-border/20" style={{ gridTemplateColumns: `56px repeat(${cols.length}, 1fr)`, height: `${ROW_PX}px` }}>
                <div className="text-[10px] text-gray-500 text-right pr-2 pt-1">
                  {hour === 0 ? "12 AM" : hour < 12 ? `${hour} AM` : hour === 12 ? "12 PM" : `${hour - 12} PM`}
                </div>
                {cols.map(b => (
                  <div key={b.id} className="border-l border-border/20" />
                ))}
              </div>
            ))}
    ...
    if (isMobile) {
      ...
      return (
        <div className="flex flex-col h-full">
          ...
          <div className="grid grid-cols-7 border-b border-border bg-surface-raised/30 flex-shrink-0">
            {weekDays.map(day => {
              ...
              return (
                <button key={dateStr} onClick={() => setCurrentDate(day)}
                  className="py-2 text-center hover:bg-surface-raised/50 transition-colors">
                  <p className={cn("text-[10px] uppercase tracking-wider", today ? "text-gold" : "text-gray-400")}>
                  ...
                  <p className={cn(
                    ...
                    isSelected ? "bg-gold text-black" : today ? "text-gold" : "text-white",
                  )}>
                    {day.getDate()}
                  </p>
    ...
    return (
      <div ref={scrollRef} className="overflow-auto h-full">
        <div className="min-w-[700px]">
          <div className="grid sticky top-0 z-10 bg-surface border-b border-border" style={{ gridTemplateColumns: `56px repeat(7, 1fr)` }}>
            <div />
            {weekDays.map(day => {
              const dateStr = formatDateForDb(day);
              const dayAppts = appointments.filter(a => a.date === dateStr);
              ...
              return (
                <button key={dateStr} onClick={() => setAgendaDate(day)}
                  className={cn("py-2 text-center border-l border-border/40 hover:bg-surface-raised/30 transition-colors", today && "bg-gold/5")}>
                  <p className={cn("text-[10px] uppercase tracking-wider", today ? "text-gold" : "text-gray-400")}>
          ...
          <div className="relative">
            {HOURS_24.map(hour => (
              <div key={hour} className="grid border-b border-border/20" style={{ gridTemplateColumns: `56px repeat(7, 1fr)`, height: `${ROW_PX}px` }}>
                <div className="text-[10px] text-gray-500 text-right pr-2 pt-1">
                  {hour === 0 ? "12 AM" : hour < 12 ? `${hour} AM` : hour === 12 ? "12 PM" : `${hour - 12} PM`}
                </div>
                {weekDays.map(day => (
                  <div key={formatDateForDb(day)} className={cn("border-l border-border/20", isToday(day) && "bg-gold/5")} />
                ))}
              </div>
            ))}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 sm:p-6 pb-3 border-b border-border space-y-3 sm:space-y-0 sm:flex sm:items-center sm:justify-between sm:gap-4">
        ...
        <div className="flex bg-surface-raised border border-border rounded-xl p-1 gap-1 self-start sm:self-auto">
      ...
      {barbers.length > 1 && (
        <div className="px-4 sm:px-6 py-2 border-b border-border/50 flex flex-wrap gap-x-4 gap-y-1.5">
          {barbers.map((b, i) => (
