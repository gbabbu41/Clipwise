...
export default function CheckInPage() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto pb-28 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-foreground uppercase tracking-wide">Check-in</h1>
        <p className="text-sm text-grey mt-0.5">Clock your team in and out, and review their history.</p>
      </div>

      {bioAvailable ? (
        <label className="flex items-center gap-3 rounded-xl border border-border bg-card p-3 cursor-pointer">
        ...
        <div className="flex items-start gap-3 rounded-xl border border-border bg-card p-3">
          ...
          <p className="text-xs text-grey">Face ID / fingerprint check-in works on the ClipWise app. On the web you clock staff in and out manually here.</p>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-border bg-card p-10 text-center text-grey text-sm">No active staff yet. Add staff first.</div>
      ) : (
          ...
          {barbers.map(b => {
            ...
            return (
              <div key={b.id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3">
                <div className="w-10 h-10 rounded-full bg-card-raised text-foreground font-bold flex items-center justify-center flex-shrink-0 overflow-hidden">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className={cn("text-xs mt-0.5", open ? "text-emerald-400" : "text-grey")}>
      ...
      <div>
        <div className="flex items-center gap-2 mb-2"><Clock size={15} className="text-grey" /><h2 className="text-sm font-bold text-foreground">Recent check-in history</h2></div>
        {history.length === 0 ? (
          <div className="rounded-2xl border border-border bg-card p-8 text-center text-grey text-sm">No check-in records yet</div>
        ) : (
          <div className="rounded-2xl border border-border bg-card overflow-hidden divide-y divide-border">
            {history.map(h => (
              <div key={h.id} className="flex items-center gap-3 px-4 py-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-foreground truncate">{nameOf(h.barber_id)}</p>
                  <p className="text-xs text-grey">{h.date}</p>
                </div>
                <div className="text-right text-xs">
                  <p><span className="text-emerald-400">{h.clock_in}</span> <span className="text-grey-muted">→</span> <span className="text-rose-400">{h.clock_out ?? "—"}</span></p>
                  <p className="text-grey mt-0.5">{h.clock_out ? `${h.hours_worked ?? 0}h` : "Clocked in"}</p>
                </div>
              </div>
            ))}
          </div>
