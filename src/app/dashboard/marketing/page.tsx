        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            htmlBody: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;">
              <h2 style="color:#C9A84C;margin-bottom:16px;">${shop?.name ?? "Your Barber"}</h2>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Email campaigns to grow your client base</p>
        </div>
        {tab === "campaigns" && (
      ...
      {tab === "campaigns" && (
        <div className="space-y-6">
          {/* Quick actions */}
          <div>
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Quick Campaigns</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {[
                ...
                <button
                  ...
                  className="text-left p-4 bg-surface border border-border rounded-2xl hover:border-gold/30 transition-all group"
                >
                  <div className="flex items-start gap-3">
                    <span className="text-2xl">{qa.icon}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-white group-hover:text-gold transition-colors">{qa.label}</p>
                      <p className="text-xs text-gray-400 mt-0.5 leading-relaxed">{qa.desc}</p>
                    </div>
                    <ChevronRight size={14} className="text-gray-600 group-hover:text-gold transition-colors flex-shrink-0 mt-0.5" />
                  </div>
                </button>
              ))}
            </div>
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {MOCK_HISTORY.map(c => (
                      <tr key={c.id} className="border-b border-border/50 hover:bg-surface-raised/50 transition-colors">
                        <td className="px-3 py-3 text-sm font-medium text-white">{c.name}</td>
                        <td className="px-3 py-3 text-xs text-gray-400">{c.segment}</td>
                        <td className="px-3 py-3 text-xs text-gray-400">{c.sentAt}</td>
      ...
      {tab === "create" && (
        <div className="space-y-6">
          <button onClick={() => setTab("campaigns")} className="text-sm text-gray-400 hover:text-white transition-colors flex items-center gap-1">
            ← Back to campaigns
          </button>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-5">
              <Card>
                <CardHeader><Zap size={18} className="text-gold" /><CardTitle>Campaign Setup</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  ...
                  {/* Segment picker */}
                  <div>
                    <p className="text-sm font-medium text-gray-300 mb-2">Audience Segment</p>
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              selectedSegment.id === seg.id
                                ...
                                : "border-border text-gray-400 hover:border-border/80"
                            )}
                          >
              ...
              <Card>
                <CardHeader><Mail size={18} className="text-gold" /><CardTitle>Message</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  {/* Template picker */}
                  <div>
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            selectedTemplate.id === t.id
                              ...
                              : "border-border text-gray-400 hover:text-white hover:border-border/80"
                          )}
                        >
                          {t.label}
                        </button>
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      Use <span className="font-mono bg-surface-raised px-1 rounded">{"{name}"}</span>, <span className="font-mono bg-surface-raised px-1 rounded">{"{shop}"}</span>, <span className="font-mono bg-surface-raised px-1 rounded">{"{link}"}</span> as placeholders
                    </p>
                  </div>
            ...
            <div className="space-y-4">
              <Card>
                <CardHeader><Users size={18} className="text-gold" /><CardTitle>Audience Summary</CardTitle></CardHeader>
                <CardContent className="space-y-3">
                  <div className="p-4 bg-surface-raised rounded-xl border border-border text-center">
                    <p className="text-4xl font-bold text-white">{recipientsWithEmail.length}</p>
                    <p className="text-xs text-gray-400 mt-1">recipients with email</p>
              ...
              <Card>
                <CardHeader><Clock size={18} className="text-gold" /><CardTitle>Delivery</CardTitle></CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    <button className="w-full p-3 rounded-xl border border-gold bg-gold/10 text-sm font-medium text-gold text-left">
                      ● Send Now
                    </button>
                    <button
                      className="w-full p-3 rounded-xl border border-border text-sm text-gray-400 text-left hover:border-border/80"
              ...
              {(!subject.trim() || !body.trim()) && (
                <p className="text-xs text-amber-400 text-center">Fill in subject and message to send</p>
              )}
            </div>
          </div>
        </div>
