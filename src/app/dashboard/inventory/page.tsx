    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold text-gold mt-1">{formatCurrency(totalValue)}</p>
        </Card>
        <Card className="p-4">
          ...
          {CATEGORIES.map(cat => (
            <button
              ...
              className={cn("px-3 py-1.5 text-xs rounded-lg border font-medium transition-colors",
                catFilter === cat ? "bg-gold/15 border-gold/30 text-gold" : "border-border text-gray-400 hover:text-white")}
            >
              {cat}
            </button>
          ))}
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filtered.map(item => {
                    ...
                    return (
                      <tr key={item.id} className={cn("border-b border-border/50 hover:bg-surface-raised/20 transition-colors", isLow && "bg-red-500/5")}>
                        {isEditing ? (
                          <>
                            <td className="px-3 py-2">
                              <input value={editRow.name} onChange={e => setEditRow(p => p && ({ ...p, name: e.target.value }))}
                                className="w-full bg-background border border-gold/40 rounded-lg px-2 py-1.5 text-sm text-white focus:outline-none" />
                            </td>
                            <td className="px-3 py-2">
                              <select value={editRow.category} onChange={e => setEditRow(p => p && ({ ...p, category: e.target.value }))}
                                className="bg-background border border-border rounded-lg px-2 py-1.5 text-sm text-white focus:outline-none">
                                {CATEGORIES.filter(c => c !== "All").map(c => <option key={c} value={c}>{c}</option>)}
                              </select>
                            </td>
                            <td className="px-3 py-2">
                              <input value={editRow.price} onChange={e => setEditRow(p => p && ({ ...p, price: e.target.value }))} type="number" min="0" step="0.01"
                                className="w-20 bg-background border border-border rounded-lg px-2 py-1.5 text-sm text-white focus:outline-none" />
                            </td>
                            <td className="px-3 py-2">
                            ...
                            <td className="px-3 py-2">
                              <div className="flex items-center gap-1">
                                <input value={editRow.quantity} onChange={e => setEditRow(p => p && ({ ...p, quantity: e.target.value }))} type="number" min="0"
                                  className="w-16 bg-background border border-border rounded-lg px-2 py-1.5 text-sm text-white focus:outline-none" />
                            ...
                            <td className="px-3 py-2">
                              <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => setEditRow(null)} className="text-gray-400 hover:text-white transition-colors p-1" title="Cancel">
                                  <X size={16} />
                                </button>
                              </div>
                            </td>
                          ...
                          <>
                            ...
                            <td className="px-3 py-3 text-sm text-gold">{formatCurrency(item.price)}</td>
                            <td className="px-3 py-3 text-sm text-gray-400">{item.cost_price ? formatCurrency(item.cost_price) : "—"}</td>
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-2">
                                <button onClick={() => adjustQty(item.id, -1)} className="w-6 h-6 rounded-md bg-surface-raised border border-border text-white hover:border-red-400/50 hover:text-red-400 flex items-center justify-center text-xs transition-colors">
                                  <TrendingDown size={12} />
                                </button>
                                <span className={cn("text-sm font-bold w-8 text-center", isLow ? "text-red-400" : "text-white")}>
                                  {item.quantity}
                                </span>
                                <button onClick={() => adjustQty(item.id, 1)} className="w-6 h-6 rounded-md bg-surface-raised border border-border text-white hover:border-emerald-400/50 hover:text-emerald-400 flex items-center justify-center text-xs transition-colors">
                                  <TrendingUp size={12} />
                                </button>
                                <span className="text-xs text-gray-600">/ {item.low_stock_threshold}</span>
                              </div>
                            </td>
                            <td className="px-3 py-3 text-sm text-gray-300">{formatCurrency(item.price * item.quantity)}</td>
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-1">
                                <button onClick={() => startEdit(item)} className="text-gray-400 hover:text-white transition-colors p-1" title="Edit">
                                  <Pencil size={14} />
                                </button>
                                <button onClick={() => setDeleteId(item.id)} className="text-gray-400 hover:text-red-400 transition-colors p-1" title="Delete">
                                  <Trash2 size={14} />
                                </button>
                              </div>
                            </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-surface border border-border rounded-2xl p-6 w-full max-w-md space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs text-gray-400">Starting Qty</label>
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-surface border border-border rounded-2xl p-6 w-full max-w-sm space-y-4">
              ...
              <p className="text-sm text-gray-400">This product will be permanently deleted and removed from your inventory.</p>
