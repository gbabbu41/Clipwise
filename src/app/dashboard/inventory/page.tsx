    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[#8f8f8f] mt-0.5">Manage products and stock levels</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          <p className="text-xs text-[#8f8f8f]">Retail Value</p>
          <p className="text-2xl font-bold text-white mt-1">{formatCurrency(totalValue)}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-[#8f8f8f]">Cost Value</p>
          ...
          {CATEGORIES.map(cat => (
            <button
              ...
              className={cn("px-3 py-1.5 text-xs rounded-lg border font-medium transition-colors",
                catFilter === cat ? "bg-black/10 border-black text-white" : "border-[#2a2a2a] text-[#8f8f8f] hover:text-white")}
            >
              {cat}
            </button>
          ))}
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="py-16 text-center">
              <Package size={40} className="mx-auto mb-4 text-[#8f8f8f]" />
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filtered.map(item => {
                    ...
                    return (
                      <tr key={item.id} className={cn("border-b border-[#2a2a2a]/50 hover:bg-[#141414]/20 transition-colors", isLow && "bg-red-500/5")}>
                        {isEditing ? (
                          <>
                            ...
                            <td className="px-3 py-2">
                              <div className="flex items-center gap-1">
                                <input value={editRow.quantity} onChange={e => setEditRow(p => p && ({ ...p, quantity: e.target.value }))} type="number" min="0"
                                  className="w-16 bg-black border border-[#2a2a2a] rounded-lg px-2 py-1.5 text-sm text-white focus:outline-none" />
                            ...
                            <td className="px-3 py-2">
                              <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => setEditRow(null)} className="text-[#8f8f8f] hover:text-white transition-colors p-1" title="Cancel">
                                  <X size={16} />
                                </button>
                              </div>
                            </td>
                          ...
                          <>
                            ...
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => adjustQty(item.id, 1)} className="w-6 h-6 rounded-md bg-[#141414] border border-[#2a2a2a] text-white hover:border-emerald-400/50 hover:text-emerald-400 flex items-center justify-center text-xs transition-colors">
                                  <TrendingUp size={12} />
                                </button>
                            ...
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => setDeleteId(item.id)} className="text-[#8f8f8f] hover:text-red-400 transition-colors p-1" title="Delete">
                                  <Trash2 size={14} />
                                </button>
                              </div>
                            </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto overscroll-contain [&>*]:my-auto">
            <div className="bg-black shadow-sm border border-[#2a2a2a] rounded-2xl p-6 w-full max-w-md space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs text-[#8f8f8f]">Starting Qty</label>
                  <input value={form.quantity} onChange={e => setForm(p => ({ ...p, quantity: e.target.value }))} type="number" min="0" placeholder="0"
                    className="w-full bg-[#141414] border border-[#2a2a2a] rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-[#8f8f8f] focus:outline-none focus:border-black" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs text-[#8f8f8f]">Low Stock Alert At</label>
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto overscroll-contain [&>*]:my-auto">
            <div className="bg-black shadow-sm border border-[#2a2a2a] rounded-2xl p-6 w-full max-w-sm space-y-4">
              ...
              <p className="text-sm text-[#8f8f8f]">This product will be permanently deleted and removed from your inventory.</p>
