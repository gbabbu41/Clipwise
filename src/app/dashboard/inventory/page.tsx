  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground uppercase tracking-wide">Inventory</h1>
          <p className="text-sm text-grey mt-0.5">Manage products and stock levels</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className={cn("p-4", lowStock.length > 0 && "border-red-500/30")}>
          <p className="text-xs text-grey">Low Stock</p>
          <div className="flex items-center gap-2 mt-1">
            <p className={cn("text-2xl font-bold", lowStock.length > 0 ? "text-red-400" : "text-foreground")}>{lowStock.length}</p>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="py-16 text-center">
              <Package size={40} className="mx-auto mb-4 text-grey" />
                ...
                <thead>
                  <tr className="border-b border-border">
                    {["Product", "Category", "Retail", "Cost", "Stock", "Value", "Actions"].map(h => (
                      <th key={h} className={cn("text-left text-xs font-medium text-grey px-3 py-3", ["Category", "Cost", "Value"].includes(h) && "hidden md:table-cell")}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={item.id} className={cn("border-b border-[#2a2a2a]/50 hover:bg-card-raised/20 transition-colors", isLow && "bg-red-500/5")}>
                        {isEditing ? (
                          <>
                            ...
                            <td className="px-3 py-2 hidden md:table-cell">
                              <select value={editRow.category} onChange={e => setEditRow(p => p && ({ ...p, category: e.target.value }))}
                                className="bg-card border border-border rounded-lg px-2 py-1.5 text-sm text-foreground focus:outline-none">
                                {CATEGORIES.filter(c => c !== "All").map(c => <option key={c} value={c}>{c}</option>)}
                              </select>
                            </td>
                            <td className="px-3 py-2">
                              <input value={editRow.price} onChange={e => setEditRow(p => p && ({ ...p, price: e.target.value }))} type="number" min="0" step="0.01"
                                className="w-20 bg-card border border-border rounded-lg px-2 py-1.5 text-sm text-foreground focus:outline-none" />
                            </td>
                            <td className="px-3 py-2 hidden md:table-cell">
                            ...
                            <td className="px-3 py-2">
                              <div className="flex items-center gap-1">
                                <input value={editRow.quantity} onChange={e => setEditRow(p => p && ({ ...p, quantity: e.target.value }))} type="number" min="0"
                                  className="w-16 bg-card border border-border rounded-lg px-2 py-1.5 text-sm text-foreground focus:outline-none" />
                                <span className="text-xs text-grey">/ {editRow.low_stock_threshold}</span>
                              </div>
                            </td>
                            <td className="px-3 py-2 hidden md:table-cell">
                            ...
                            <td className="px-3 py-2">
                              <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => setEditRow(null)} className="text-grey hover:text-foreground transition-colors p-1" title="Cancel">
                                  <X size={16} />
                                </button>
                              </div>
                            </td>
                          ...
                          <>
                            ...
                            <td className="px-3 py-3 text-sm text-foreground">{formatCurrency(item.price)}</td>
                            <td className="px-3 py-3 text-sm text-grey hidden md:table-cell">{item.cost_price ? formatCurrency(item.cost_price) : "—"}</td>
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-2">
                                <button onClick={() => adjustQty(item.id, -1)} className="w-6 h-6 rounded-md bg-card-raised border border-border text-foreground hover:border-red-400/50 hover:text-red-400 flex items-center justify-center text-xs transition-colors">
                                  <TrendingDown size={12} />
                                </button>
                                <span className={cn("text-sm font-bold w-8 text-center", isLow ? "text-red-400" : "text-foreground")}>
                                  {item.quantity}
                                </span>
                                <button onClick={() => adjustQty(item.id, 1)} className="w-6 h-6 rounded-md bg-card-raised border border-border text-foreground hover:border-emerald-400/50 hover:text-emerald-400 flex items-center justify-center text-xs transition-colors">
                                  <TrendingUp size={12} />
                                </button>
                                <span className="text-xs text-grey">/ {item.low_stock_threshold}</span>
                              </div>
                            </td>
                            <td className="px-3 py-3 text-sm text-grey hidden md:table-cell">{formatCurrency(item.price * item.quantity)}</td>
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-1">
                                <button onClick={() => startEdit(item)} className="text-grey hover:text-foreground transition-colors p-1" title="Edit">
                                  <Pencil size={14} />
                                </button>
                                <button onClick={() => setDeleteId(item.id)} className="text-grey hover:text-red-400 transition-colors p-1" title="Delete">
                                  <Trash2 size={14} />
                                </button>
                              </div>
                            </td>
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto overscroll-contain [&>*]:my-auto">
            <div className="bg-card shadow-sm border border-border rounded-2xl p-6 w-full max-w-sm space-y-4">
              ...
              <p className="text-sm text-grey">This product will be permanently deleted and removed from your inventory.</p>
