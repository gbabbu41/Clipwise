...
export default function ReviewsPage() {
  ...
  if (!shop) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-sm text-grey">Reviews will appear here once your shop is active.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-foreground uppercase tracking-wide">Reviews</h1>
        <p className="text-sm text-grey mt-0.5">Monitor and respond to client feedback</p>
      </div>

      {loading ? (
        <div className="grid md:grid-cols-2 gap-6">
          {[1,2].map(i => <div key={i} className="h-44 rounded-2xl bg-card-raised animate-pulse" />)}
        </div>
      ) : (
        <>
          {/* Rating Overview */}
              ...
              <CardContent>
                <div className="flex items-center gap-6">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-grey mt-1">{reviews.length} review{reviews.length !== 1 ? "s" : ""}</p>
                  </div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-sm text-grey">Replied to reviews</p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <span className="text-sm text-grey self-center">{filtered.length} review{filtered.length !== 1 ? "s" : ""}</span>
          </div>
          ...
          <div className="space-y-4">
            {filtered.length === 0 ? (
              ...
              <Card key={review.id}>
                ...
                {review.comment && <p className="text-sm text-grey leading-relaxed mb-3">&ldquo;{review.comment}&rdquo;</p>}

                {review.reply && (
                  <div className="p-3 bg-card-raised rounded-xl border border-border mb-3">
                    <p className="text-xs text-foreground mb-1">Your reply:</p>
                    <p className="text-sm text-grey">{review.reply}</p>
                  </div>
                )}
                ...
                <div className="flex gap-2">
                  {!review.reply ? (
                    ...
                    <Button variant="ghost" size="sm" className="text-grey" onClick={() => { setReplyingTo(replyingTo === review.id ? null : review.id); setReplyText(review.reply ?? ""); }}>
                      Edit Reply
                    </Button>
                  )}
                ...
                {replyingTo === review.id && (
                  <div className="mt-4 space-y-2">
                    <textarea value={replyText} onChange={e => setReplyText(e.target.value)}
                      ...
                      className="w-full rounded-xl border border-border bg-card-raised px-4 py-2.5 text-sm text-foreground placeholder:text-grey focus:outline-none focus:ring-2 focus:ring-black/20 resize-none"
                      rows={3} />
