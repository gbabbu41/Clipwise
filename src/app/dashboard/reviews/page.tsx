  ...
  if (!shop) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-sm text-gray-500">Reviews will appear here once your shop is active.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Monitor and respond to client feedback</p>
      </div>

      {loading ? (
        <div className="grid md:grid-cols-2 gap-6">
          {[1,2].map(i => <div key={i} className="h-44 rounded-2xl bg-gray-100 animate-pulse" />)}
        </div>
      ) : (
        <>
          {/* Rating Overview */}
                  ...
                  <div className="flex-1 space-y-2">
                    {ratingBreakdown.map(r => (
                      <div key={r.stars} className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-500 w-3">{r.count}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-sm text-gray-500">Replied to reviews</p>
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex rounded-xl border border-gray-200 overflow-hidden">
              {[["all","All"],["5","5★"],["4","4★"],["below3","Below 3★"]].map(([v,l]) => (
                <button key={v} onClick={() => setRatingFilter(v)}
                  className={cn("px-3 py-2 text-sm font-medium transition-colors", ratingFilter === v ? "bg-gold text-black" : "text-gray-500 hover:text-gray-900 bg-gray-100")}>
                  {l}
                </button>
              ))}
            </div>
            ...
            <span className="text-sm text-gray-500 self-center">{filtered.length} review{filtered.length !== 1 ? "s" : ""}</span>
          </div>
            ...
            {filtered.length === 0 ? (
              <Card>
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">{reviews.length === 0 ? "No reviews yet. Reviews will appear here when clients rate their experience." : "No reviews match your filters."}</p>
                </div>
              </Card>
            ) : filtered.map(review => (
              <Card key={review.id}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-gray-900 font-semibold">{review.client_name ?? "Anonymous"}</p>
                ...
                {review.comment && <p className="text-sm text-gray-600 leading-relaxed mb-3">&ldquo;{review.comment}&rdquo;</p>}

                {review.reply && (
                  <div className="p-3 bg-gray-100 rounded-xl border border-gray-200 mb-3">
                ...
                {replyingTo === review.id && (
                  <div className="mt-4 space-y-2">
                    <textarea value={replyText} onChange={e => setReplyText(e.target.value)}
                      ...
                      className="w-full rounded-xl border border-gray-200 bg-gray-100 px-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-black/20 resize-none"
                      rows={3} />
