...
// `native` is resolved on the SERVER (from the request User-Agent) by the layout
// server-wrapper and handed down here, so the trial banner knows it's in the
// native app at SSR time — it can never be server-rendered in the app, regardless
// of any future change to the loading-spinner gating below. See layout.tsx.
export default function DashboardLayoutClient({ children, native }: { children: React.ReactNode; native: boolean }) {
