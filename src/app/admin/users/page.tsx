...
function RoleBadge({ role }: { role: UserRole }) {
  const map: Record<UserRole, string> = {
    ...
    customer: "text-gray-400 bg-gray-500/15 border-gray-500/30",
  };
          ...
          {(["all", "shop_owner", "barber", "customer"] as RoleFilter[]).map(r => {
            ...
            return (
              <div key={r} className="bg-surface border border-border rounded-xl px-4 py-3 text-center min-w-[80px]">
                ...
                <p className="text-xs text-gray-500 capitalize">{roleLabels[r]}</p>
              </div>
            );
          })}
        </div>
          ...
          {ROLES.map(r => (
            <button key={r} onClick={() => setRoleFilter(r)}
              className={cn("px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-all border",
                roleFilter === r ? "bg-gold text-black border-gold" : "border-border text-gray-400 hover:text-white hover:border-gray-500")}>
              {roleLabels[r]}
            </button>
          ))}
        </div>
                ...
                <tbody>
                  {filtered.map(u => (
                    <tr key={u.id} className="border-b border-border/50 hover:bg-surface-raised/30 transition-colors">
                      ...
                      <td className="px-3 py-3 text-sm text-gray-400">{u.phone || "—"}</td>
                      <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">{u.created_at?.slice(0, 10) ?? "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <p className="text-xs text-gray-600 mt-3 px-3">Showing {filtered.length} of {users.length} users</p>
          </CardContent>
        </Card>
      )}
    </div>
