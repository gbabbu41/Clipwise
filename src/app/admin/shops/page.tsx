  ...
  return (
    <div className="p-4 lg:p-8 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-[#8f8f8f] mt-0.5">All registered shops on the platform</p>
      </div>
          ...
          {STATUS_FILTERS.map(f => (
            <button key={f} onClick={() => setStatusFilter(f)}
              className={cn("px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-all border",
                statusFilter === f ? "bg-gold text-black border-gold" : "border-border text-[#6e6e6e] hover:text-white hover:border-gray-500")}>
      ...
      {loading ? (
        ...
        <Card>
          <div className="py-20 text-center space-y-3">
            ...
            <p className="text-sm text-[#8f8f8f] max-w-xs mx-auto">
                ...
                <thead>
                  <tr className="border-b border-border">
                    {["Shop Name", "Owner", "City", "Plan", "Status", "Joined", "Actions"].map(h => (
                      <th key={h} className="text-left text-xs font-medium text-[#6e6e6e] px-3 py-2 whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(s => (
                    <tr key={s.id} onClick={() => router.push(`/admin/shops/${s.id}`)} className="border-b border-border/50 hover:bg-surface-raised/30 transition-colors cursor-pointer">
                      <td className="px-3 py-3">
                        ...
                        <p className="text-xs text-[#8f8f8f]">/book/{s.slug}</p>
                      </td>
                      <td className="px-3 py-3">
                        <p className="text-sm text-gray-300">{s.users?.name ?? "—"}</p>
                        <p className="text-xs text-[#8f8f8f]">{s.users?.email ?? s.email}</p>
                      </td>
                      ...
                      <td className="px-3 py-3 text-xs text-[#8f8f8f] whitespace-nowrap">{formatDate(s.created_at.slice(0, 10))}</td>
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto overscroll-contain [&>*]:my-auto">
            <div className="bg-surface border border-border rounded-2xl p-6 w-full max-w-md space-y-4">
              ...
              <p className="text-sm text-[#6e6e6e]">Rejecting <span className="text-white font-medium">{rejectModal.name}</span>. Provide a reason:</p>
              <textarea
                value={rejectReason}
