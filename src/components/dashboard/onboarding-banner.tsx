      ...
      <div className="mb-6 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 flex items-center justify-between animate-fade-in">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#777]">Your booking page is live. Start sharing it with clients.</p>
          </div>
        </div>
        <button
  ...
  return (
    <div className="mb-6 bg-surface border border-gold/20 rounded-2xl p-5 animate-fade-in">
      <div className="flex items-start justify-between mb-4">
        ...
        <button onClick={dismissProgress} className="text-[#777] hover:text-gray-300 transition-colors p-1 touch-target" aria-label="Dismiss setup guide">
          <X size={14} />
        </button>
      </div>
        ...
        {steps.map((step, i) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className={cn("text-sm font-medium", step.done ? "text-[#777] line-through" : "text-white")}>{step.label}</p>
              <p className="text-xs text-[#777] truncate">{step.description}</p>
            </div>
            {!step.done && <ArrowRight size={14} className="text-[#777] group-hover:text-gold transition-colors flex-shrink-0" />}
          </Link>
        ))}
      </div>
    </div>
