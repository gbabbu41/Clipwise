...
export function Calendar({ value, onChange, minDate, maxDate, isDateDisabled, renderDayBadge, onMonthChange, className }: CalendarProps) {
  ...
  return (
    <div className={cn("bg-[#0c0c0c] border border-[#2a2a2a] rounded-2xl p-4 w-full max-w-xs", className)}>
      {/* Header */}
      <div className="flex items-center justify-between px-1 mb-2">
        <button
          ...
          className="w-9 h-9 rounded-full hover:bg-[#141414] text-[#8f8f8f] hover:text-white flex items-center justify-center transition-colors"
        >
          <ChevronLeft size={18} />
        </button>
      ...
      <div className="grid grid-cols-7 mb-1">
        {WEEKDAYS.map((w, i) => (
          <span key={i} className="text-[10px] uppercase tracking-wider text-[#8f8f8f] text-center py-1.5 font-semibold">{w}</span>
        ))}
      </div>
